...
board_t * makeBoard(int w, int h, int numMines) {
  //malloc and initialize a board_t representing the board.
   board_t * initialBoard = (board_t *) malloc(sizeof(board_t));
   initialBoard->width = w;
   initialBoard->height = h;
   initialBoard->totalMines = numMines;
   initialBoard->board = (int **)malloc(h * sizeof(int *));
   //allocate space for the 2D array "board".
   for (int i = 0; i < h; i++){
     initialBoard->board[i] = (int *)malloc(w * sizeof(int));
   }
   //fill in all squares on the board with UNKNOWN
   for (int i = 0; i < h; i++){
     for (int j = 0; j < w; j++){
       initialBoard->board[i][j] = UNKNOWN;
     }
   }
   //call addRandomMine an appropriate number of times
   for (int i = 0; i < numMines; i++){
     addRandomMine(initialBoard);
   }
   return initialBoard;
}
...
int boundsCheck(int x, int y, board_t * b){
  int hEdge = b->height - 1;
  int wEdge = b->width - 1;
  return (((y >= 0) && (y <= hEdge)) && ((x >= 0) && (x <= wEdge)));
}

int countMines(board_t * b, int x, int y) {
  int count = 0;
  //count the mines in the 8 squares around that (x, y) coordinate
  //IS_MINE(b->board[ny][nx])
  count += boundsCheck(x - 1, y - 1, b) && IS_MINE(b->board[y - 1][x - 1]);
  count += boundsCheck(x - 1, y , b) && IS_MINE(b->board[y][x - 1]);
  count += boundsCheck(x - 1, y + 1, b) && IS_MINE(b->board[y + 1][x - 1]);
  count += boundsCheck(x, y - 1, b) && IS_MINE(b->board[y - 1][x]);
  count += boundsCheck(x, y + 1, b) && IS_MINE(b->board[y + 1][x]);
  count += boundsCheck(x + 1, y - 1, b) && IS_MINE(b->board[y - 1][x + 1]);
  count += boundsCheck(x + 1, y, b) && IS_MINE(b->board[y][x + 1]);
  count += boundsCheck(x + 1, y + 1, b) && IS_MINE(b->board[y + 1][x + 1]);
...
//the game is won when no squares are UNKNOWN
